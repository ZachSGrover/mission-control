...
def save_json(path: Path, data):
    """Atomarer Schreibe + Retry bei OneDrive-Locks (Errno 22 auf Windows).
    Schreibt erst in eine .tmp-Datei und führt anschließend einen atomic-replace —
...
def already_blasted_keys(blast_log: dict, sender_user_id: str, cooldown_hours: float = 24.0) -> set:
    """Dedup-Keys (id:<num> oder h:<handle>) für Empfänger die INNERHALB der
    letzten <cooldown_hours> Stunden bereits geblastet wurden.

    cooldown_hours <= 0  → keine Zeitfilter (alle Einträge zählen wie früher).
    Default 24h          → nach einem Tag wird der gleiche Empfänger wieder eligible.
    """
    keys = set()
    cutoff = None
    ...
    for e in blast_log.get(sender_user_id, []):
        if cutoff is not None:
            ts = (e.get("ts") or "").rstrip("Z")
            try:
                ts_dt = datetime.datetime.fromisoformat(ts)
            except (ValueError, TypeError):
                # Kein parsbares Datum → Eintrag als "alt" werten, NICHT skippen
                continue
            if ts_dt < cutoff:
                # Eintrag ist älter als der Cooldown → Empfänger ist wieder eligible
                continue
...
def main():
    ...
    print(f"\n{'═'*55}")
    print(f"  blast_bot.py  —  Start: {now_str}{' (DRY RUN)' if dry_run else ''}")
    print(f"{'═'*55}")
    ...
    if dry_run:
        ...
        source      = (auftrag or {}).get("source_user_id", "")
        source_type = ((auftrag or {}).get("source_type") or "contacts").lower()
        message     = (auftrag or {}).get("message", "") or ""
        # Count recipients without exposing them
        recip_count = 0
        try:
            if source_type == "followers":
                fl = load_json(FOLLOWERS_PATH, {})
                recip_count = len(fl.get(source) or [])
            else:
                contacts_local = load_json(CONTACTS_PATH, {})
                recip_count = len(contacts_local.get(source) or [])
        except Exception:
            pass
        n_accounts = len(accs)
        planned    = n_accounts * recip_count
        print(f"DRY RUN — blast_bot.py would:")
        print(f"  - source_type: {source_type}")
        print(f"  - source key present: {bool(source)}")
        print(f"  - recipient pool count: {recip_count}")
        print(f"  - sender accounts: {n_accounts}")
        print(f"  - message length: {len(message)} chars (preview redacted)")
        print(f"  - planned DMs: up to {planned}  (senders x recipients)")
        print(f"  - would call: ads_open -> playwright -> compose/profile -> send DM")
    ...
    # Cooldown: nach wie vielen Stunden ist ein bereits geblasteter Empfänger
    # WIEDER eligible? Default 24h. 0 = unbegrenzte Sperre (alter Modus).
    cooldown_hours = float(auftrag.get("cooldown_hours") if auftrag.get("cooldown_hours") is not None else 24.0)
    ...
    print(f"  Cooldown: {cooldown_hours}h (Empfänger nach Ablauf wieder eligible)" if cooldown_hours > 0
          else "  Cooldown: ∞ (Empfänger werden NIE wieder geblastet)")
    ...
    # accounts, recipients, daily_cap, batch_size werden HART auf 1 gecapped.
    ...
    if live_mode:
        ...
        if confirm != "YES" or mta != 1:
            log_msg = [
                ...
                "   Blast-Run wurde sicherheitshalber abgebrochen.",
            ]
        ...
        if bad:
            ...
            return
        # HART-CAP: 1 sender, 1 recipient, batch=1, daily_cap=1
        cap_log = []
        if len(accounts) > 1:
            cap_log.append(f"⚠ LIVE-CAP: {len(accounts)} sender accounts → cap to 1")
            accounts = accounts[:1]
        if len(recipients) > 1:
            cap_log.append(f"⚠ LIVE-CAP: {len(recipients)} recipients → cap to 1")
            recipients = recipients[:1]
        if batch_size > 1:
            cap_log.append(f"⚠ LIVE-CAP: batch_size={batch_size} → 1")
            batch_size = 1
        if daily_cap > 1:
            cap_log.append(f"⚠ LIVE-CAP: daily_cap={daily_cap} → 1")
            daily_cap = 1
        cap_log.append("━━ LIVE MODE ACTIVE — single-shot: 1 sender × 1 recipient = max 1 action ━━")
    ...
    for acc in accounts:
        ...
        done = already_blasted_keys(blast_log, uid, cooldown_hours)
        ...
        if skipped:
            cd_label = f" (innerhalb {int(cooldown_hours)}h)" if cooldown_hours > 0 else " (alle bisherigen Blasts)"
            log.append(f"{nm}: {skipped} bereits geblastet{cd_label}, übersprungen.")
        if day_start:
